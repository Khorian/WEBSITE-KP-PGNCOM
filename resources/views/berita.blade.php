...
<body class="bg-gray-50 text-gray-800 font-sans" x-data="{ 
    ...
    <section class="py-12 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <!-- Grid Kartu Berita (Dinamis Database + Filter Alpine.js) -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @forelse ($beritas as $item)
                @php
                    // Normalisasi kategori dari database agar cocok dengan string lowercase alpine ('infrastruktur', 'kegiatan', dll)
                    $catSlug = strtolower(trim($item->kategori));
                @endphp
                
                <div x-show="(selectedCategory === 'semua' || selectedCategory === '{{ $catSlug }}') && ('{{ addslashes($item->judul) }}'.toLowerCase().includes(searchQuery.toLowerCase()))" 
                    ...
                    <div class="p-6 pt-0 flex items-center justify-between text-xs">
                        <!-- Tombol Pop-up Detail Berita -->
                        <button @click="isOpen = true; currentNews = {
                            title: '{{ addslashes($item->judul) }}',
                            date: '{{ $item->created_at->format("d M Y") }}',
                            category: '{{ $item->kategori }}',
                            image: '{{ $item->gambar ? asset($item->gambar) : asset("images/news-1.png") }}',
                            content: `{{ addslashes(strip_tags($item->konten)) }}`
